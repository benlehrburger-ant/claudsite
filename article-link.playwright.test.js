...
test.describe('Article Link Validation', () => {
  ...
  test('Claude Opus 4.5 article card has valid href attribute', async ({ page }) => {
    ...
    await expect(articleCard).toHaveAttribute('href', '/articles/claude-opus-4-5');
  });
  ...
  test('Claude Opus 4.5 article card is clickable', async ({ page }) => {
    ...
    await expect(articleCard.locator('.card-title')).toHaveText('Introducing Claude Opus 4.5');
  ...
  test('Claude Opus 4.5 article card navigates to correct page when clicked', async ({ page }) => {
    // Find and click the first article card
    const articleCard = page.locator('.cards-grid .card').first();
    await articleCard.click();

    // Wait for navigation and verify the URL
    await expect(page).toHaveURL('http://localhost:3000/articles/claude-opus-4-5');
  });

  test('all article cards in Latest Updates have valid href attributes', async ({ page }) => {
